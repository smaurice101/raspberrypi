...
def generatedoc(**context):    
   ...
   # dockerrun = re.escape(dockerrun) 
    ...
    collection = context['ti'].xcom_pull(task_ids='step_9_solution_task_ai',key="{}_vectordbcollectionname".format(sname))    
    ...
    privategptrun = "docker run -d -p {}:{} --net=host --gpus all --env PORT={} --env GPU=1 --env COLLECTION={} --env WEB_CONCURRENCY={} --env CUDA_VISIBLE_DEVICES={} {}".format(pgptport,pgptport,pgptport,collection,concurrency,pgptcontainername)
    ...
    doparse("/{}/docs/source/details.rst".format(sname), ["--consumefrom--;{}".format(consumefrom)])
    doparse("/{}/docs/source/details.rst".format(sname), ["--pgpt_data_topic--;{}".format(pgpt_data_topic)])
    doparse("/{}/docs/source/details.rst".format(sname), ["--vectordbcollectionname--;{}".format(vectordbcollectionname)])
    doparse("/{}/docs/source/details.rst".format(sname), ["--offset--;{}".format(offset[1:])])
    doparse("/{}/docs/source/details.rst".format(sname), ["--rollbackoffset--;{}".format(rollbackoffset[1:])])
    doparse("/{}/docs/source/details.rst".format(sname), ["--topicid--;{}".format(topicid[1:])])
    doparse("/{}/docs/source/details.rst".format(sname), ["--enabletls--;{}".format(enabletls[1:])])
    doparse("/{}/docs/source/details.rst".format(sname), ["--partition--;{}".format(partition[1:])])
    doparse("/{}/docs/source/details.rst".format(sname), ["--prompt--;{}".format(prompt)])
    doparse("/{}/docs/source/details.rst".format(sname), ["--context--;{}".format(context)])
    doparse("/{}/docs/source/details.rst".format(sname), ["--jsonkeytogather--;{}".format(jsonkeytogather)])
    doparse("/{}/docs/source/details.rst".format(sname), ["--keyattribute--;{}".format(keyattribute)])
    doparse("/{}/docs/source/details.rst".format(sname), ["--concurrency--;{}".format(concurrency[1:])])
    doparse("/{}/docs/source/details.rst".format(sname), ["--cuda--;{}".format(cuda[1:])])
